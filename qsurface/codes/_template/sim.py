...
class PerfectMeasurements(ABC):
    ...
    def random_errors(self, apply_order: Optional[List[str]] = None, measure: bool = True, **kwargs):
        ...
        """
        self.instance = time.time()
        ordered_errors = [self.errors[name] for name in apply_order] if apply_order else self.errors.values() #Getting the errors
...
class FaultyMeasurements(PerfectMeasurements):
    ...
    def random_errors(
        ...
        **kwargs,
        ...
        self.random_errors_layer(**kwargs)
        self.random_measure_layer() # Last layer is with perfect measurements!
